...
%%
%% now do what-if on the anomaly points:
times_factors = [1.1 1.2 1.3];
for i = 1: length(times_factors)
    times_factor = times_factors(i);
    [I_real_re_wi, I_imag_re_wi] = tensor2data_anomaly_whatif(re_myTensor_prd, V_real2, V_imag2, dataname,err_timepts,times_factor, length(V_real1));
    
    err_sum_whatif(i) = computeErr(I_real2(err_timepts), I_real_re_wi(err_timepts))+computeErr(I_imag2(err_timepts), I_imag_re_wi(err_timepts));
end


[vvv, iii] = min(err_sum_whatif);
times_factor = times_factors(iii);
[I_real_re_wi, I_imag_re_wi] = tensor2data_anomaly_whatif(re_myTensor_prd, V_real2, V_imag2, dataname,err_timepts,times_factor, length(V_real1));
...
subplot(2,1,1); hold on;
h1 = plot(I_real2(1:t_tot),'color',[0.5 0.5 0.5], 'linewidth',2); hold on;
% h2 = plot((t_tot+1:t_tot2),I_real2(t_tot+1:end),'o','color',[0.5 0.5 0.5],  'markersize',3);
h2 = plot((t_tot+1:t_tot2),I_real2(t_tot+1:end),'.k','markersize',5);
h3 = plot((t_tot+1:t_tot2),I_real_re_wi(t_tot+1:end),'-r', 'linewidth',2);
set(gca,'XTick',[1:24:t_tot2]);
set(gca,'XTickLabel',label_tmp);
set(gca,'XTickLabelRotation',90);
for i = 1: ceil(t_tot2/24)
...
h1 = plot(I_imag2(1:t_tot),'color',[0.5 0.5 0.5],'linewidth',2); hold on;
h2 = plot((t_tot+1:t_tot2),I_imag2(t_tot+1:end),'.k', 'markersize',5);
h3 = plot((t_tot+1:t_tot2),I_imag_re_wi(t_tot+1:end),'-r', 'linewidth',2);
set(gca,'XTick',[1:24:t_tot2]);
set(gca,'XTickLabel',label_tmp2);
set(gca,'XTickLabelRotation',90);
for i = 1: ceil(t_tot2/24)
...
ylabel('I_i [A]', 'fontsize', fontsize_in);
if 1
    saveas(gcf, [data_out_folder dataname '_anomaly2_whatif'],'png');
end
...
figure;
hold on;
h2 = plot(I_real2(t_tot+1:end),'.k','markersize',20);
h3 = plot(I_real_re_wi(t_tot+1:end),'-r', 'linewidth',5);
for i = 1: ceil((t_tot2-t_tot)/24)
    plot([(i-1)*24 (i-1)*24], [min(I_real1) max(I_real1)], ':k');
end
for i = 1: length(err_timepts)
    f1 = [1 2 3 4];
    v1 = [err_timepts(i) min(I_real1); err_timepts(i) max(I_real1) ;err_timepts(i)+1 max(I_real1);err_timepts(i)+1 min(I_real1)];
...
set(gca, 'XTickLabel', []);
if 1
    saveas(gcf, [data_out_folder dataname '_anomaly2_whatif_mag_re'],'png');
end
...
figure;
hold on;
h2 = plot(I_imag2(t_tot+1:end),'.k','markersize',20);
h3 = plot(I_imag_re_wi(t_tot+1:end),'-r', 'linewidth',5);
for i = 1: ceil((t_tot2-t_tot)/24)
    plot([(i-1)*24 (i-1)*24], [min(I_imag1) max(I_imag1)], ':k');
end
for i = 1: length(err_timepts)
    f1 = [1 2 3 4];
    v1 = [err_timepts(i) min(I_imag1); err_timepts(i) max(I_imag1) ;err_timepts(i)+1 max(I_imag1);err_timepts(i)+1 min(I_imag1)];
...
set(gca, 'XTickLabel', []);
if 1
    saveas(gcf, [data_out_folder dataname '_anomaly2_whatif_mag_im'],'png');
end
