function [I_real_re, I_imag_re] = tensor2data_whatif(myTensor, V_real, V_imag, data_proc_method, dataname, Gfactor, Bfactor)
...
for day_idx = 1: n_d
    ...
    if data_proc_method == 1
        for tt = 1: t_d
            % retrieve parameters
            G_now = myTensor(day_idx, tt,1) * Gfactor;
            alpha_r_now = myTensor(day_idx, tt,2);
            B_now = myTensor(day_idx, tt,3) * Bfactor;
