function [V_real1, I_real1, V_imag1, I_imag1, myTensor, V_real2, I_real2, V_imag2, I_imag2, n_d, label_tmp_unit, label_tmp, label_tmp2, t_d] = processData_segmentation(...
    dataname, n_d_pred, n_d_removed, sigma, window_size, method_funcs, method_idx, BIC_params, trial_idx)
...
myTensor = construct_tensor_segmentation(V_real,I_real,V_imag,I_imag,n_d,t_d,method_funcs,method_idx, BIC_params, trial_idx);
...
figure;
subplot(4,1,1); plot(G_mat);hold on; title('G');
for i = 1: ceil(length(G_mat)/24)
    plot([0+(i-1)*24 0+(i-1)*24], [min(G_mat) max(G_mat)], ':m');
end
subplot(4,1,2); plot(alpha_r_mat);hold on;title('alpha_r');
for i = 1: ceil(length(G_mat)/24)
    plot([0+(i-1)*24 0+(i-1)*24], [min(alpha_r_mat) max(alpha_r_mat)], ':m');
end
subplot(4,1,3); plot(B_mat);hold on;title('B');
for i = 1: ceil(length(G_mat)/24)
    plot([0+(i-1)*24 0+(i-1)*24], [min(B_mat) max(B_mat)], ':m');
end
subplot(4,1,4); plot(alpha_i_mat); hold on;title('alpha_i');
for i = 1: ceil(length(G_mat)/24)
